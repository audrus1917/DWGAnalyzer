...
class DrawingProcessor:
    ...
    def parse_drawing_sources(
        ...
        processed_sources = (process_source(_source) for _source in drawing_sources)

        created_entities = 0
        if not dry:
            created_entities = DrawingStore.save_to_db(
                sources_path,
                processed_sources,
...
def collect_drawing_summary(drawing: Drawing) -> dict[str, Any]:
    ...
    logger.debug(_("Layout count: %d"), len(drawing.layouts))
    logger.debug(_("Block count: %d"), len(drawing.blocks))
    logger.debug(_("Layer count: %d"), len(drawing.layers))

    total_primitives: int = 0
    for layout in drawing.layouts:
        total_primitives += len(layout)
    logger.debug(_("Primitive count across layouts: %d"), total_primitives)
    ...
    for layout in drawing.layouts:
        layouts.append({
            ...
            "data": {
                ...
                "taborder": safe_float(layout.dxf.get("taborder", None)),
            }
        })
    ...
    for layer in drawing.layers:
        ...
        layers.append({
            ...
            "data": {
                ...
                "linetype": str(layer.dxf.get("linetype", "")) or None,
                "lineweight": safe_float(layer.dxf.get("lineweight", None)),
                "on": not layer.is_off(),
    ...
    return {
        ...
        "layers": layers,
        "blocks": blocks,
        "primitives": collect_layout_entities(drawing),
        "block_links": block_links,
        "block_layers": block_layers,
    }


def process_source(entry: DrawingSource) -> DrawingSource:
...
class DrawingStore:
    """Store processed drawing summaries into the database."""
    ...
    def save_to_db(
        ...
        with session_factory() as session:
            ...
            for entry in processed_entries:
                ...
                primitives = list(summary.get("primitives", []))
                ...
                for idx, primitive in enumerate(primitives, start=1):
                    ...
                    if "data" in primitive_payload and isinstance(primitive_payload["data"], (dict, list)):
                        primitive_entity.data = primitive_payload["data"]
...
__all__ = ["DrawingProcessor", "DrawingStore"]
